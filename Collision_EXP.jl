...
const FLAG= "RResults_t="							# FLAG Output
...
for i=0:1:Int(round(N_barrier*Ne))                                                       	
        Barrier0[Ne*(Nz+1)-i,Ne*(Nz+1)-i] =  0.042*abs(sin(1*pi*(i/(N_barrier*Ne))))	        	# 0.042, Lz = 08.75, Lb = 1.25
end
...
############################################# Exponential Method #############################################################
@time begin	# time evolution operator U(Ng*dt) with the absorption potential
	U_dt = Barrier*LinearAlgebra.exp(-im*Ng*dt*H_eff - 2*im*Ng*neta*Delta2 -Ng*dt*Barrier0)*Barrier
end
Barrier0 = 0
Barrier  = 0
H_eff 	 = 0
...
file_l=open(FLAG*string(0)*"Total_","w")
